...
class NCAAA_CS_GeneratorTests(unittest.TestCase):
    ...
    def test_ee490_generates_and_coop_remains_excluded(self):
        with tempfile.TemporaryDirectory() as directory:
            result = generator.generate_course(self.course("EE490"), Path(directory))
            self.assertEqual(result.status, "generated")
        excluded = generator.generate_course(self.course("EE492"), Path(tempfile.mkdtemp()))
    ...
    def test_coop_is_excluded(self):
        result = generator.generate_course(self.course("EE492"), Path(tempfile.mkdtemp()))
