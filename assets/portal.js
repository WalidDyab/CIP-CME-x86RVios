...
function filterCourses(courses,q,track){q=(q||'').toLowerCase(); return courses.filter(c=>{const n=normCourse(c); const text=[n.code,n.title,n.track,n.re,n.desc,JSON.stringify(n.clos)].join(' ').toLowerCase(); return (!q||text.includes(q))&&(!track||track==='All'||n.track===track||n.re===track||n.level===track);})}
function setupCourseBrowser(courses, opts={}){let state={q:'',track:'All'}; const list=byId('courseList'), search=byId('search'), track=byId('trackFilter'); const norms=courses.map(normCourse); if(track){const vals=['All',...[...new Set(norms.flatMap(c=>[c.track,c.re,c.level]).filter(Boolean))].sort()]; track.innerHTML=vals.map(v=>`<option>${esc(v)}</option>`).join(''); track.onchange=()=>{state.track=track.value; render()}} if(search){search.oninput=()=>{state.q=search.value; render()}} function render(){const rows=filterCourses(courses,state.q,state.track); byId('resultCount')&&(byId('resultCount').textContent=rows.length+' courses'); list.innerHTML=rows.map(c=>courseCard(c,opts.linkPrefix||'course-dashboard.html?course=')).join('') || '<div class="alert">No matching courses.</div>'} render()}
function renderCLOTable(clos){return `<div class="table-wrap"><table><thead><tr><th>No.</th><th>CLO</th><th>Mapping</th></tr></thead><tbody>${(clos||[]).map(x=>`<tr><td class="code">${esc(x.no||x.clo_number||'')}</td><td>${esc(x.text||x.clo_text||'')}</td><td>${esc(x.plos||[...(x.mapped_sos||[]),...(x.pi_codes||[])].join(', '))}</td></tr>`).join('')}</tbody></table></div>`}
...
window.portal={loadJSON,esc,badge,normCourse,countMsc,renderStats,setupCourseBrowser,renderCLOTable,renderTopics,getParam};
