(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    content().addEventListener('change', event => {
      const action=event.target.dataset.existingAction; if(action!==undefined){ state.reviewer=document.getElementById('reviewerName').value; state.existing[+action].action=event.target.value; render(); return; }
    ...
    content().addEventListener('click', event => {
      if(event.target.id==='addNewClo'){ state.reviewer=document.getElementById('reviewerName').value; state.added.push({id:state.nextId++,proposedText:'',sos:[],pis:[],justification:''}); render(); }
